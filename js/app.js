(function () {
    ...
    setTimeout(() => {
      ...
      objectStore.openCursor().onsuccess = (e) => {
        ...
        if (cursor) {
          ...
          tdNameEmail.className =
            "px-6 py-4 whitespace-no-wrap border-b border-gray-200";
          pName.className =
            "text-sm leading-5 font-medium text-gray-700 text-lg font-bold";
          ...
          tdCompany.className =
            "px-6 py-4 whitespace-no-wrap border-b border-gray-200 leading-5 text-gray-700";
          ...
          tdActions.className =
            "px-6 py-4 whitespace-no-wrap border-b border-gray-200 leading-5 text-sm cursor-pointer";
  ...
  function deleteClient(idClient) {
    ...
    const request = objectStore.get(idClient);
    let clientName = "";
    request.onerror = () => {
      console.log("There was an error with this transaction");
    };
    ...
    setTimeout(() => {
      const confirmDelete = confirm(
        `Are you sure to delete this client: ${clientName}`
      );
      if (confirmDelete) {
        const transaction = DB.transaction(["crm"], "readwrite");
        const objectStore = transaction.objectStore("crm");
        ...
        transaction.onerror = () => {
          console.log('There was an error deleting the client');
        }

        transaction.oncomplete = () => {
          const dataClient = document.querySelector(`[data-cliente="${idClient}"]`);
          dataClient.parentElement.parentElement.remove();
        }
      }
    }, 200);
  }
